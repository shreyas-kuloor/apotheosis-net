    ...
    IOptions<MediaRequestSettings> mediaRequestOptions) : IMediaRequestService
{
    ...
    public async Task<IEnumerable<MovieDto>> SearchMovies(string searchTerm)
    {
        ...
        var movieDtos = movies.Take(_mediaRequestSettings.OptionCount).Select(m =>
        {
            MediaStatus status;
    ...
    public async Task<IEnumerable<SeriesDto>> SearchSeries(string searchTerm)
    {
        ...
        var seriesDtos = series.Take(_mediaRequestSettings.OptionCount).Select(s => {
            MediaStatus status;
    ...
    public async Task RequestMovie(string title, int tmdbId)
    {
        ...
        var request = new MovieRequest
        {
            ...
            Monitored = true,
            QualityProfileId = qualityProfiles.FirstOrDefault(q => q.Name == _mediaRequestSettings.QualityProfileName)?.Id ?? 4,
            AddOptions = new()
            {
                SearchForMovie = true,
            },
